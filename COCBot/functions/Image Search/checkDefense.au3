...
Global $debugDef = 0 ; set 1 to ON debug mode, set 0 to OFF debug mode
...
Func checkDefenseIT()
	...
	Local $defSimilarity[1][10]=[[0.92,0.92,0.91,0.92,0.92,0.91,0.92,0.92,0.92]]

	$skipITBase = False
	$numDefFound = 0;

	If $debugDef = 1 Then
	SetLog("Checking Inferno Tower", $COLOR_BLUE)
	EndIf
	...
	$t = 0
		For $i = 0 To 9
			If FileExists($trapTH[$t][$i]) Then
				...
				If IsArray($res) Then
					...
					Else
						...
						If $chkInferno = 1 AND $DefText[$t] = "Inferno Tower" Then
							If ($Defx > 1 AND $Defx < 800) AND ($Defy > 1 AND $Defy < 720) Then
								$skipITBase = True							
								If $debugDef = 1 Then
								Setlog($DefText[$t] & " Found at" & " " & $Defx & " " & $Defy )
								EndIf
							Else
								$skipITBase = False
								If $debugDef = 1 Then
								Setlog($DefText[$t] & " Not Found")
								EndIf
							EndIf
						EndIf	
					EndIf
			...
			EndIf
		Next
EndFunc ;checkDefenseIT

Func checkITBase()
	If _GUICtrlComboBox_GetCurSel($cmbDetectITBase) <> 0 Then
		checkDefenseIT()

		If $skipITBase = True Then
		$skipITBase = False
		Switch  _GUICtrlComboBox_GetCurSel($cmbDetectITBase)
			Case 0
			;do nothing
			Case 1
			;skip Inferno Tower live base
				Setlog("~~~Inferno Tower detected.. Skipping Base~~~", $COLOR_ORANGE)
				Return True
			Case 2
			;attack Inferno Tower live base
				Setlog("~~~Inferno Tower detected.. Let's do this!~~~", $COLOR_ORANGE)
				Return False
		EndSwitch
		EndIf
	EndIf
