...
[ _
[@ScriptDir & "\images\TH\9.bmp", 70, 0, 14], _								;0    TH 9's
...
[@ScriptDir & "\images\TH\Th9_6.png", 60, 14, 7], _ 	;Good 75			;7
[@ScriptDir & "\images\TH\Th9_7.bmp", 75, -1, 13], _	;Good 75			;8
[@ScriptDir & "\images\TH\Th9_8.bmp", 75, 2, -5], _		;Good 75			;9
[@ScriptDir & "\images\TH\Th9_9.bmp", 60, -22, 11], _	;Good 75			;10
			   ...
			   If FileExists($atkTHADV[$t][$i][0]) Then
					 ...
					 If FilterTH()=True Then
					  If $THLocation = 1 and $t >= 1 Then ; add this 'And $OptTrophyMode = 1 ' to used two factor only throphie mode
					     ...
					     Endif
							;capture and save a missed th imag
#cs

							If $t = 2  And $i > 13   Then
								SetLog("Taking snapshot...", $COLOR_GREEN)
								Local $Date = @YEAR & "-" & @MON & "-" & @MDAY
								Local $Time = @HOUR & "." & @MIN & "." & @SEC

 								_CaptureRegion($THx - 50, $THy - 50, $THx + 50, $THy + 50)
								$NoTHimage = $t&"-"&$i&"--"&$THx&"  "&$THy&"  "& $Time & ".png"
								_GDIPlus_ImageSaveToFile($hBitmap, @ScriptDir&"\THoffset\"&$t&"\"&$NoTHimage)
								_GDIPlus_ImageDispose($hBitmap)

							ElseIf $t = 3  And $i > 23    Then
								SetLog("Taking snapshot...", $COLOR_GREEN)
								Local $Date = @YEAR & "-" & @MON & "-" & @MDAY
								Local $Time = @HOUR & "." & @MIN & "." & @SEC
...
#ce
					  ...
					  EndIf
					   If $THLocation = 1 Then
						...
						$THx += $atkTHADV[$t][$i][2]
						$THy += $atkTHADV[$t][$i][3]
						;OverlayCircle($THx, $THy, 5, 0xFFFF0000, 2)
					     ...
					     Endif
						   ...
						   Endif
						    ...
						    If $THLocation = 1 then
							   ...
							   ;capture and save a missed th image
#cs


							If $ct = 2  And $z > 13 Then
								SetLog("Taking snapshot...", $COLOR_GREEN)
								Local $Date = @YEAR & "-" & @MON & "-" & @MDAY
								Local $Time = @HOUR & "." & @MIN & "." & @SEC

 								_CaptureRegion($CTHx - 50, $CTHy - 50, $CTHx + 50, $CTHy + 50)
								$NoTHimage = $ct&"-"&$z&"- "&$CTHx&"  "&$CTHy&"  "& $Time & ".png"
								_GDIPlus_ImageSaveToFile($hBitmap, @ScriptDir&"\THoffset\"&$ct&"\"&$NoTHimage)
								_GDIPlus_ImageDispose($hBitmap)

							ElseIf $ct = 3  And  $z >23   Then
								SetLog("Taking snapshot...", $COLOR_GREEN)
								Local $Date = @YEAR & "-" & @MON & "-" & @MDAY
								Local $Time = @HOUR & "." & @MIN & "." & @SEC
